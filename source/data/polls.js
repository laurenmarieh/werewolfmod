...
const getAll = async () => {
  db.query('SELECT * FROM polls where is_closed = false ORDER BY id ASC').then(results => {
    return results.rows
  })
}
...
const getByChannel = async req => {
  ...
  return db
    .query('SELECT * FROM polls where team_id= $1 and channel_id =$2 and is_closed = $3 LIMIT 1', [
      teamId,
      channelId,
      isClosed
    ])
...
const createPoll = async request => {
  const { pollTitle, isClosed, channelName, channelId, teamName, teamId, isGame, choices } = request
  return db
    .query(
      'INSERT INTO public.polls (poll_title, is_closed, channel_name, channel_id, team_name, team_id, is_game, choices)' +
        'VALUES($1, $2, $3, $4, $5, $6, $7, $8)',
      [pollTitle, isClosed, channelName, channelId, teamName, teamId, isGame, {}]
    )
    .then(pollResults => {
      return getByChannel({
        teamId: teamId,
        ...
        return db
          .query(
            'INSERT INTO public.poll_options (poll_id, option_index, option_name)' + queryValues
          )
          .then(optionResults => {
...
const closePoll = async req => {
  ...
  return db
    .query(
      'UPDATE polls set is_closed = true, closed_date = now() where team_id = $1 and channel_id =$2 and is_closed = false returning * ',
      [teamId, channelId]
    )
    .then(results => {
